...
if(isset($_SESSION['usuario'])) {
    ...
    $consultaTiposTrabajo = $db->prepare("SELECT t.*, o.nombre AS nombreorg FROM tipostrabajo t JOIN organizaciones o WHERE t.organizacion_id = o.id");
    $consultaTiposTrabajo->execute();
    $tiposTrabajo = $consultaTiposTrabajo->fetchAll(PDO::FETCH_ASSOC);

    if(isset($_POST['editar'])){
        $_SESSION['tipoTrabajoEditar'] = $tiposTrabajo[$_POST['editar']];
        header('Location: editarTipoTrabajo.php');
    } elseif(isset($_POST['eliminar'])){

        $consultaDelete = $db->prepare("DELETE FROM tipostrabajo WHERE id = :id");
        $consultaDelete->execute([
            ':id' => $tiposTrabajo[$_POST['eliminar']]['id']
        ]);
                    ...
                    <tbody>
                        ...
                        <?php
                            if(count($tiposTrabajo) == 0){
                                ...
                                echo "<td>".$tipo['nombreorg'] ?? "---"."</td>";
                                echo "<td>".$tipo['descripcion']."</td>";
                                echo "<td>
                                    <button id='editar' name='editar' value='".$key."'>Editar</button>
                                    <button id='eliminar' name='eliminar' value='".$key."'>Eliminar</button>
                                    </td>";
                                echo "</tr>";
                                }
                            }
